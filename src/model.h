...
static inline bool sd_version_is_bagel(SDVersion version) {
    return version == VERSION_BAGEL;
}

static inline bool sd_version_uses_flux_vae(SDVersion version) {
    if (sd_version_is_bagel(version) || sd_version_is_flux(version) || sd_version_is_z_image(version) || sd_version_is_boogu_image(version) || sd_version_is_longcat(version)) {
        return true;
    }
    return false;
}
